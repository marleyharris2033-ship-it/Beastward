// Beastward v9: direct Stage 1 artwork with safe component-aware cropping
(() => {
  const VERSION='20260911-stage1-direct-v9';
  const sources={
  ...
  function makeTransparentCrop(img,index,count){
    ...
    for(let y=0;y<h;y++)for(let x=0;x<w;x++){const i=y*w+x,p=i*4,r=d[p],g=d[p+1],b=d[p+2],lo=Math.min(r,g,b),hi=Math.max(r,g,b);if(lo>244&&hi-lo<12){d[p+3]=0;continue;}if(lo>232&&hi-lo<9)d[p+3]=Math.min(d[p+3],Math.max(0,(246-lo)*20));if(d[p+3]>42)solid[i]=1;}
    // Connected components are used only to identify the main artwork cluster.
    const seen=new Uint8Array(w*h),components=[],stack=[];
    for(let y=0;y<h;y++)for(let x=0;x<w;x++){const start=y*w+x;if(!solid[start]||seen[start])continue;let minX=x,maxX=x,minY=y,maxY=y,n=0;seen[start]=1;stack.push(start);while(stack.length){const q=stack.pop(),qx=q%w,qy=(q/w)|0;n++;if(qx<minX)minX=qx;if(qx>maxX)maxX=qx;if(qy<minY)minY=qy;if(qy>maxY)maxY=qy;for(let yy=Math.max(0,qy-1);yy<=Math.min(h-1,qy+1);yy++)for(let xx=Math.max(0,qx-1);xx<=Math.min(w-1,qx+1);xx++){const ni=yy*w+xx;if(solid[ni]&&!seen[ni]){seen[ni]=1;stack.push(ni);}}}components.push({minX,maxX,minY,maxY,n});}
    components.sort((a,b)=>b.n-a.n);
    let main=components[0]||{minX:0,maxX:w-1,minY:0,maxY:h-1,n:w*h};
    // Preserve nearby effects belonging to the main beast, but reject detached miniature duplicates.
    const mainCx=(main.minX+main.maxX)/2,mainCy=(main.minY+main.maxY)/2,mainW=main.maxX-main.minX+1,mainH=main.maxY-main.minY+1;
    const keep=components.filter((o,k)=>{if(k===0)return true;const cx=(o.minX+o.maxX)/2,cy=(o.minY+o.maxY)/2,dx=Math.abs(cx-mainCx),dy=Math.abs(cy-mainCy);return o.n>=main.n*.025&&dx<mainW*.72&&dy<mainH*.72;});
    let minX=w,minY=h,maxX=-1,maxY=-1;for(const o of keep){minX=Math.min(minX,o.minX);minY=Math.min(minY,o.minY);maxX=Math.max(maxX,o.maxX);maxY=Math.max(maxY,o.maxY);}
    const margin=Math.max(5,Math.round(Math.max(maxX-minX+1,maxY-minY+1)*.035));minX=Math.max(0,minX-margin);minY=Math.max(0,minY-margin);maxX=Math.min(w-1,maxX+margin);maxY=Math.min(h-1,maxY+margin);
    // Clear everything outside the selected main-artwork crop. This removes the small duplicate and sheet debris.
    for(let y=0;y<h;y++)for(let x=0;x<w;x++)if(x<minX||x>maxX||y<minY||y>maxY)d[(y*w+x)*4+3]=0;
    c.putImageData(frame,0,0);
    const sw=maxX-minX+1,sh=maxY-minY+1,out=document.createElement('canvas');out.width=256;out.height=256;const o=out.getContext('2d');o.imageSmoothingEnabled=true;const scale=Math.min(240/sw,240/sh),dw=Math.max(1,Math.round(sw*scale)),dh=Math.max(1,Math.round(sh*scale));o.drawImage(work,minX,minY,sw,sh,Math.round((256-dw)/2),Math.round((256-dh)/2),dw,dh);return out.toDataURL('image/png');
  }
  async function install(){for(const source of Object.values(sources)){const sheet=await loadImage(source.file);source.ids.forEach((id,index)=>{if(!beasts[id])return;const url=makeTransparentCrop(sheet,index,source.ids.length);stage1Urls[id]=url;const img=new Image();img.src=url;stage1Images[id]=img;beasts[id].sprite=url;beasts[id].towerSprite=url;spriteImgs[id]=img;});}
    ...
    try{renderStarters();}catch(e){}try{renderCollection();}catch(e){}try{renderBestiary();}catch(e){}try{choices();}catch(e){}document.documentElement.dataset.stage1RosterArt='ready-direct-v9';}
  install().catch(error=>{console.error('Stage 1 direct art failed',error);document.documentElement.dataset.stage1RosterArt='failed-direct-v9';});
})();