// Beastward v60: exact user-approved Stage 2 evolution artwork
(() => {
  const sheet='assets/pixel/evolved/stage2_sheet.png?v=60';
  ...
  stageSpriteMarkup=function(id,stage=evolutionStage(id),extra='',unseen=false){
    ...
    return `<span class="stage-sprite stage-2 type-${b.type.toLowerCase()} ${unseen?'unseen-sprite':''} ${extra}" aria-label="${unseen?'Undiscovered beast':name}" style="position:relative;overflow:hidden;display:inline-block;">
      <img src="${sheet}" alt="${unseen?'Undiscovered beast':name}" style="position:absolute;max-width:none!important;max-height:none!important;width:500%!important;height:400%!important;left:-${c.col*100}%;top:-${c.row*100}%;object-fit:fill!important;transform:none!important;filter:${unseen?'brightness(0) saturate(0) contrast(1.2)':'drop-shadow(0 3px 3px #0007)'};image-rendering:auto;">
    </span>`;
  };
