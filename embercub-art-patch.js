// Beastward approved Embercub four-stage PNG art
(() => {
  const ART_VERSION='20260910-embercub-redesign-v1';
  const id='embercub';
  if(!beasts[id])return;
  beasts[id].sprite=`assets/pixel/redesign/embercub_1.png?v=${ART_VERSION}`;
  ...
  spritePathForStage=function(beastId,stage=1){
    if(beastId===id){const s=Math.max(1,Math.min(4,stage));return `assets/pixel/redesign/embercub_${s}.png?v=${ART_VERSION}`;}
