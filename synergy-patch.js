...
// Individual-beast aware. Must load after individual/loadout/combat-stat patches.
(() => {
  const synergyDefs = [
  ...
  function speciesFor(id){
    if(beasts?.[id]) return id;
    const i=(save.beastInstances||[]).find(x=>x&&x.uid===id);
    return i?.species && beasts[i.species] ? i.species : null;
  }
  const typesFor = ids => new Set((ids||[]).map(speciesFor).filter(Boolean).map(id=>beasts[id].type));
  const battleIds = () => (save.lastInstanceLoadout||[]).length ? save.lastInstanceLoadout : (battleLoadout||[]);
  const teamIds = () => synergyBattleActive ? battleIds() : (loadoutDraft||[]);
  const activeSynergies = (ids = teamIds()) => {
    const types=typesFor(ids);
    return synergyDefs.filter(s=>s.types.every(t=>types.has(t)));
  };
  const hasSynergy = id => synergyBattleActive && activeSynergies(battleIds()).some(s=>s.id===id);

  window.BeastwardSynergies={
    definitions:synergyDefs,
    active:()=>activeSynergies().map(s=>s.id),
    activeFor:ids=>activeSynergies(ids).map(s=>s.id)
  };
  ...
  style.textContent=`
    ...
    .synergy-preview-head b{font-size:12px;letter-spacing:.12em;color:#f2db82}.synergy-preview-head small{color:#9fb2a7;font-size:10px}
    .synergy-list{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:7px}.synergy-card{display:flex;gap:8px;align-items:flex-start;padding:8px;border-radius:9px;background:#10251a;border:1px solid #6ecf8f45}
    .synergy-card .synergy-icon{font-size:17px;line-height:1}.synergy-card b{display:block;font-size:11px;color:#e9d37b}.synergy-card small{display:block;font-size:9px;line-height:1.3;color:#b9c8be;margin-top:2px}
    .synergy-card.suggestion{background:#101914;border-color:#ffffff14;opacity:.82}.synergy-card.suggestion b{color:#b7c6bc}.synergy-card.suggestion em{display:block;font-style:normal;color:#80d7a1;font-size:9px;margin-top:2px}
    .synergy-empty{padding:8px 9px;border-radius:9px;background:#ffffff08;color:#9eafa4;font-size:10px}.battle-synergy-strip{margin:0 0 10px;padding:9px;border-radius:10px;background:#08150faa;border:1px solid #ffffff14}
    .battle-synergy-strip>small{display:block;color:#9fb0a5;font-size:9px;letter-spacing:.12em;margin-bottom:5px}.battle-synergy-chip{display:flex;align-items:center;gap:6px;padding:5px 7px;margin-top:4px;border-radius:7px;background:#153321;color:#f2df91;font-size:10px;border:1px solid #78d29235}.battle-synergy-chip span{font-size:13px}
  ...
  function ensureLoadoutPanel(){
    const shell=document.querySelector('.loadout-card-shell');if(!shell)return null;
    let panel=document.querySelector('#synergyPreview');
    if(!panel){panel=document.createElement('div');panel.id='synergyPreview';panel.className='synergy-preview';const grid=document.querySelector('#loadoutGrid');if(grid)shell.insertBefore(panel,grid)}
    return panel;
  }
  function renderSynergyPreview(){
    const panel=ensureLoadoutPanel();if(!panel)return;
    const ids=loadoutDraft||[],selectedTypes=typesFor(ids),active=activeSynergies(ids);
    const suggestions=synergyDefs.filter(s=>!active.includes(s)).map(s=>({...s,missing:s.types.filter(t=>!selectedTypes.has(t)),owned:s.types.filter(t=>selectedTypes.has(t))})).filter(s=>s.owned.length===1&&s.missing.length===1).slice(0,3);
  ...
  function ensureBattlePanel(){
    const panel=document.querySelector('.tower-panel');if(!panel)return null;
    let strip=document.querySelector('#battleSynergyStrip');
    if(!strip){strip=document.createElement('div');strip.id='battleSynergyStrip';strip.className='battle-synergy-strip';const choices=document.querySelector('#towerChoices');if(choices)panel.insertBefore(strip,choices);else panel.appendChild(strip)}
    return strip;
  }
  function renderBattleSynergies(){
    const strip=ensureBattlePanel();if(!strip)return;
    const active=activeSynergies(battleIds());
    strip.innerHTML=active.length?'<small>ACTIVE SYNERGIES</small>'+active.map(s=>'<div class="battle-synergy-chip"><span>'+s.icon+'</span><b>'+s.name+'</b></div>').join(''):'<small>ACTIVE SYNERGIES</small><div class="synergy-empty">None active</div>';
  }

  const baseRenderLoadoutPicker=renderLoadoutPicker;
  renderLoadoutPicker=function(){const r=baseRenderLoadoutPicker.apply(this,arguments);renderSynergyPreview();return r};
  ...
  const baseShow=show;
  show=function(id){if(id!=='gameScreen')synergyBattleActive=false;return baseShow.apply(this,arguments)};
  const baseReset=reset;
  reset=function(){const r=baseReset.apply(this,arguments);renderBattleSynergies();return r};

  const baseBattleStats=battleStats;
  battleStats=function(id){
    const stats=baseBattleStats(id);
    if(stats&&hasSynergy('sunstone-ward'))return{...stats,damage:stats.damage*1.08,range:clampCombatRange(stats.range*1.06)};
    return stats;
  };
  ...
  hitProjectile=function(p){
    ...
    if(hasSynergy('stormfront')){
      ...
      if(p.type==='Electric'){
        const extra=enemies.filter(e=>e!==t&&e.hp>0&&Math.hypot(e.x-t.x,e.y-t.y)<120).sort((a,b)=>Math.hypot(a.x-t.x,a.y-t.y)-Math.hypot(b.x-t.x,b.y-t.y))[0];
        if(extra){const bonus=p.damage*.35;extra.hp-=bonus;reportExtraDamage(p.beastId,bonus);fx('lightning',t.x,t.y,'#fff36c',{x2:extra.x,y2:extra.y,life:.38,maxLife:.38})}
      }
    }
  ...
  defeatEnemy=function(e){
    const shouldSpread=hasSynergy('toxic-growth')&&e&&(e.poison||0)>0,x=e?.x,y=e?.y,dps=e?.poisonDps||0;
  ...
  try{renderSynergyPreview();renderBattleSynergies()}catch(e){}
  document.documentElement.dataset.synergySystem='individual-v3';
})();