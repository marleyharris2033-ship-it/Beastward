...
(() => {
  ...
  Object.entries(FINAL_FORMS).forEach(([id,data])=>{
    if(!beasts[id])return;
    beasts[id].evo100=data.name;
  ...
  nameForStage=function(id,stage){
    const b=beasts[id];
  ...
  syncSeenStages=function(id){
    previousSyncSeenStages(id);
  ...
  Object.keys(beasts).forEach(id=>{
    ...
    evolutionSpriteImgs[id][4]=evolutionSpriteImgs[id][3]||spriteImgs[id];
  ...
  openDenBeast=function(id){
    ...
    const body=document.querySelector('#denBeastModalBody'),b=beasts[id],f=FINAL_FORMS[id];
    if(!body||!b||!f)return;
    const line=body.querySelector('.den-evo-line');
    if(line&&!line.querySelector('.final-evo-node')){
      const node=document.createElement('div');node.className='final-evo-node';
      node.innerHTML=`${stageSpriteMarkup(id,4,'den-evo-sprite')}<small>Lv 100</small><b>${f.name}</b><em>+${f.damage}% DMG • +${f.speed}% SPD • +${f.range}% RNG • +${f.special}% ELEMENT</em>`;
